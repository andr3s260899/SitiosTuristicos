import Typography from "@mui/material/Typography";
import Grid from "@mui/material/Grid";
import Card from "@mui/material/Card";
import CardActionArea from "@mui/material/CardActionArea";
import CardContent from "@mui/material/CardContent";
import CardMedia from "@mui/material/CardMedia";
import LocationOnIcon from "@mui/icons-material/LocationOn";
import VisibilityIcon from "@mui/icons-material/Visibility";
import { useNavigate } from "react-router-dom";
import { useState } from "react";
import { Button, Modal } from "antd";
import star from "../../../Layouts/Images/iconstar.png";
import { CardActions, IconButton } from "@mui/material";
import { red } from "@mui/material/colors";

export default function FeaturedPost(props: FeaturedPostProps) {
  const { post, ventana,sub } = props;
  const data =
    "";

  const [renderizar, setrenderizar] = useState<boolean>(false);

  const navigate = useNavigate();

  return (
    <>
      <body>
        <section className="mt-4">
          <div className="container">           

            <div className="row mt-3">
              <a
                href=""
                data-bs-toggle="modal"
                data-bs-target="#first"
                onClick={() => {
                  setrenderizar(true);
                }}
              >
                <div className="col-md-10 col-sm-12 col-xs-12" >
                  <div className="card_main" style={{height:(sub==='0'?300:150)}}>
                    <div className="h-100">
                      <div className="card_img_top">
                        <div className="card_img">
                          <img src={`data:image/png;base64,${data}`} />
                        </div>
                      </div>
                      <div className="card_content">
                        <div className="card_header_title">
                          <p className="text-muted">{post.pais}</p>
                        </div>
                        <div className="clearfix card_locate mb-3">
                          <p className="float-start place"> {post.lugar}</p>
                          <span className="float-end calificate">
                            <i className="fa-solid fa-star"></i>4.5
                          </span>
                        </div>
                        <div className="card_content_footer">
                          <p className="text-muted price">{post.recomendado}</p>
                          <p className="other">{post.minimo}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </a>
            </div>
          </div>
        </section>
      </body>

      {renderizar && (
        <Modal
          title={
            <p className="text-center text-dark text-uppercase mb-0 titulo modal-dialog-scrollable"></p>
          }
          okButtonProps={{ hidden: renderizar }}
          cancelButtonProps={{ hidden: renderizar }}
          visible={renderizar}
          onCancel={() => setrenderizar(false)}
          width={500}
          closable={false}
          footer={null}
        >
          <>
            <div className="h-100">
              <div className="card_img_top">
                <div className="card_img">
                  <img
                    src={`data:image/png;base64,${data}`}
                    style={{ minHeight: 450 }}
                  />
                </div>
              </div>

              <div className="card_header_title">
                <p className="text-muted"> {post.pais}</p>
              </div>
              <div className="clearfix card_locate mb-3">
                <p className="float-start place"> {post.lugar}</p>
                <span className="float-end calificate">
                  {" "}
                  <i className="fa-solid fa-star"></i> {post.puntuacion}
                </span>
                <br />
                <p style={{ margin: 9 }} className="text-justify">
                  {post.descripcion}
                </p>
              </div>
              <div className="card_content_footer">
                <p className="text-muted price">{post.recomendado}</p>
                <p className="other">{post.minimo}</p>
              </div>
              <div className="card_content_services">
                <div className="row">
                  <div className="col">
                    <p
                      style={{
                        margin: 9,
                        background: "#48aed7",
                        color: "#fff",
                        textAlign: "center",
                        borderRadius: 10,
                      }}
                    >
                      Restaurantes{" "}
                      <i
                        className="fa-solid fa-burger"
                        style={{ color: "#E58101" }}
                      ></i>{" "}
                    </p>
                  </div>
                  <div className="col">
                    <p
                      style={{
                        margin: 9,
                        background: "#48aed7",
                        color: "#fff",
                        textAlign: "center",
                        borderRadius: 10,
                      }}
                    >
                      Hoteles{" "}
                      <i
                        className="fa-solid fa-hotel"
                        style={{ color: "#EC1904" }}
                      ></i>
                    </p>
                  </div>
                  <div className="col">
                    <p
                      style={{
                        margin: 9,
                        background: "#48aed7",
                        color: "#fff",
                        textAlign: "center",
                        borderRadius: 10,
                      }}
                    >
                      Rentar carro{" "}
                      <i
                        className="fa-solid fa-car"
                        style={{ color: "#ECE104" }}
                      ></i>
                    </p>
                  </div>
                </div>
              </div>
              {sub==='0' && ( <FeaturedPost post={post} ventana={ventana} sub={'1'} ></FeaturedPost>)}
             

            </div>
            <div className="card_button_see text-center">
            {ventana === "lugar" ? (
                  <button
                    className="ml-3 btn-lg btn btn-default"
                   
                    style={{                                       
                      color: "white",
                      background: "coral",
                    }}
                    onClick={() => {
                      localStorage.setItem(
                        "seleccionado",
                        JSON.stringify(post)
                      );
                      navigate("/Mapa");
                    }}
                  >
                    <IconButton>
                      <LocationOnIcon sx={{ color: "white" }} width={5} />{" "}
                    </IconButton>
                    Localizacion
                  </button>
                ) : (
                  <button
                    className="ml-3 btn-lg btn btn-default"       
                               
                    onClick={() => {
                      localStorage.setItem(
                        "seleccionado",
                        JSON.stringify(post)
                      );
                      navigate("/pagina2");
                    }}
                  >
                    Ver mas <i className="fa-solid fa-eye"></i>
                  </button>
                )}
           

              
            </div>

          </>
        </Modal>
      )}
    </>
  );
}

interface FeaturedPostProps {
  post: {
    pais: string;
    lugar: string;
    image: string;
    recomendado: string;
    minimo: string;
    descripcion: string;
    puntuacion: string;
    opiniones: string;
    coordenadas: any;   
  };
  sub:any;
  ventana: string;
}


//////codigo viejo en caso de necesitarlo
/*<Grid item xs={3} md={3}>
              <CardActionArea
                component="a"
                onClick={() => {
                  setrenderizar(true);
                }}
              >
                <CardMedia
                  component="img"
                  sx={{ width: 350, display: { xs: "none", sm: "block" } }}
                  image={`data:image/png;base64,${data}`}
                />
                <Card
                  sx={{
                    height: "100%",
                    display: "flex",
                    flexDirection: "column",
                    width: 350,
                  }}
                >
                  <CardContent sx={{ flex: 1 }}>
                    <Typography
                      component="h2"
                      variant="subtitle1"
                      color="text.secondary"
                    >
                      {post.pais}
                    </Typography>
                    <Typography variant="h5" color="text">
                      {post.lugar}
                    </Typography>
                    <Typography variant="subtitle1">
                      {post.recomendado}
                    </Typography>
                    <Typography variant="subtitle1">{post.minimo}</Typography>
                    {ventana === "lugar" && (
                      <>
                        <div className="form-row ml-4">
                          <Button
                            className="ml-3 float-right button btn btn-default"
                            type="default"
                            htmlType="button"
                            style={{ width: 150 }}
                            onClick={() => {
                              localStorage.setItem(
                                "seleccionado",
                                JSON.stringify(post)
                              );
                              localStorage.setItem("origen", "lugar");
                              navigate("/Localizacion");
                            }}
                          >
                            <IconButton>
                              <LocationOnIcon
                                sx={{ color: red[500] }}
                                width={5}
                              />{" "}
                            </IconButton>
                            Localizacion
                          </Button>
                          <Button
                            className="ml-3 float-left button btn btn-default"
                            type="primary"
                            style={{ width: 150 }}
                            htmlType="button"
                            onClick={() => {
                              localStorage.setItem(
                                "seleccionado",
                                JSON.stringify(post)
                              );
                              navigate("/Hotel");
                            }}
                          >
                            <IconButton>
                              <VisibilityIcon
                                sx={{ color: "white" }}
                                width={5}
                              />{" "}
                            </IconButton>
                            Ver mas
                          </Button>
                        </div>
                      </>
                    )}
                  </CardContent>
                </Card>
              </CardActionArea>
            </Grid>
            */